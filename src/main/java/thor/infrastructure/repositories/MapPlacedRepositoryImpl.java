...
public class MapPlacedRepositoryImpl implements MapPlacedRepository {
    private final Map<UUID, InteractiveGameMap> data = new HashMap<>();

    private final Map<String, PhTreeSolid<UUID>> index = new HashMap<>();
    ...
    public Optional<MapPartInfo> findByLocation(Point position, String worldName) {
        ...
        if (iterator.hasNext()) {
            ...
            return Optional.of(new MapPartInfo(data.get(node.value()), new Point((int) node.lower()[0], (int) node.lower()[1], (int) node.lower()[2])));
        }
        return Optional.empty();
    }

    @Override
    public Optional<InteractiveGameMap> findById(UUID id) {
        return Optional.ofNullable(data.getOrDefault(id, null));
    }

    @Override
    public UUID addMap(InteractiveGameMap map) {
